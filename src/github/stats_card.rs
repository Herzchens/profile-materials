...
fn render_snapshot(body: &mut String, snapshot: &GitHubSnapshot, stale: bool) {
    body.push_str(
        r##"<g data-stats-primary="commits"><rect x="28" y="84" width="244" height="180" rx="18" class="primary-card"/><path d="M29 247c39-21 73-20 107-3 28 14 51 16 74-4 20-17 40-25 61-19" class="primary-wave"/>"##,
    );
    render_icon_badge(body, Icon::Commit, 46.0, 101.0, 36.0, true);
    let _ = write!(
        body,
        r##"<text x="94" y="124" class="primary-label">TOTAL COMMITS</text><text x="47" y="205" class="primary-value">{}</text></g>"##,
...
fn render_metric(
    ...
    render_icon_badge(body, metric.icon, x + 12.0, y + 10.0, 36.0, false);
    let _ = write!(
        body,
...
fn render_icon_badge(
    ...
    let radius = if circular { badge_size / 2.0 } else { 10.0 };
    let _ = write!(
        body,
        r##"<g data-stat-icon="{}" data-icon-color="{color}"><rect x="{x:.1}" y="{y:.1}" width="{badge_size:.1}" height="{badge_size:.1}" rx="{radius:.1}" fill="{color}" fill-opacity=".07" stroke="{color}" stroke-opacity=".74" stroke-width="1.1"/><svg x="{:.1}" y="{:.1}" width="18" height="18" viewBox="0 0 24 24" aria-hidden="true" fill="{color}" class="stat-icon">"##,
        icon.key(),
        x + (badge_size - 18.0) / 2.0,
        y + (badge_size - 18.0) / 2.0,
    );
    match icon {
